...
import { selectContainer, addLiquidity } from '../../../redux/slice';
...
const Pools: FC<Props> = ({ userAddress, provider, signer }) => {
      ...
      await dispatch(
        addLiquidity({
          ...
          tokenInValue: ethers.utils
            ...
            .toString(),
          tokenOutAddress,
          tokenOutValue: ethers.utils
            .parseUnits(tokenOutValue, tokenOutDecimals)
            .toString(),
          provider,
          signer,
        })
  ...
  switch (proportion.value) {
    ...
    default: {
      hint = `пропорция: ${ethers.utils.formatUnits(
        proportion.value,
        proportion.decimals
      )}`;
