...
const getPairs = createAsyncThunk(
  'Pools/getPairs',
  async ({ tokens, userAddress, provider }: Options): Promise<Pair[]> => {
    const allPairs = [];
    ...
    const existedPairs = <(Error | Pair)[]>(
      pairs.filter((pair) => pair !== null)
    );
    ...
    if (pairWithError instanceof globalThis.Error) {
      return Promise.reject(new Error(pairWithError.message));
    }

    return <Pair[]>existedPairs;
  }
);
